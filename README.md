...
const { data: transactionsData } = await balanceApi.getTransactions();
const transactions = transactionsData.value as TransactionEntity[];
</script>
```
...
export const editUser = async (user: UserEntity) =>
{
...
Все функции-запросы описываются в папке 📁 `API`, где каждый файл назван своей сущностью, условно файл `user.ts` отвечает за 
запросы связанные с юзером, `auth.ts` с авторизацией и т.д., все файлы скрепляются файлом `index.ts`, для того, чтобы 
...
const add = (a, b) => a + b;
```
Но если мы создали `helper`, который помогает нам с суммированием, то описываем его следующим образом:
```ts
/**
...
const user = userStore.user as UserEntity;
	...
	validationSchema: toTypedSchema(
		object({
			firstName: string()
				...
				.default(user.firstName || undefined),
			lastName:
				string()
					.min(2, 'Фамилия должна содержать не менее 2 символов')
					.max(16, 'Фамилия должна содержать не более 16 символов')
					.default(user.lastName || undefined),
			phone: string()
				.length(18, 'Номер телефона введен некорректно')
				.default(user.phone || undefined),
			company: string()
				.default(user.company || undefined),
			position: string()
				.default(user.position || undefined),
			email: string()
